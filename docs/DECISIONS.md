...
The button double-clicks (bubble Stop, Start Recording) are a SEPARATE mechanism with the same-ish trigger: the WKWebView's `acceptsFirstMouse` defaults false (wry `lib.rs:846`), so the click that keys a non-key window is swallowed instead of delivered to the DOM. Fixed independently via Tauri `acceptFirstMouse: true` on those window builders — no relation to the picker key-steal. **Full window audit (source-verified):** `acceptFirstMouse: true` now on `main` (config-declared in tauri.conf.json — this is WHY it was missed in the first pass; nothing else is config-declared), `webcam-bubble`, `timer-chip`, and `review`. Deliberately NOT set on: the non-interactive overlays (countdown, identify, identify-window, area-indicator — no buttons, no-op); `marquee` (drag-driven, immune per the CLAUDE.md mouseMoved gotcha, launched while the app is active); `window-picker` (its select goes to the poller instead — see below).

**Picker fix = poller (approved 2026-07-28, plan in `docs/WINDOW-PICKER-POLLER-PLAN.md`).** Drive hover AND click from a permission-free ~60 Hz `CGEventGetLocation(CGEventCreate(nil))` + `CGEventSourceCounterForEventType` poller (CoreGraphics FFI, no new crate), map global CG point -> per-display local in each overlay, drive the existing `resolveAt`. `setFocus` removed entirely (kills the key-steal at the root — the ONLY thing that depended on the overlays being key was the Esc handler, `WindowPickerOverlay.tsx:95`). **Cancel tradeoff, SETTLED — do not re-litigate:** the guaranteed cancels are the two mouse paths (on-screen Cancel button + empty-space click), poller-driven, working on every display. Esc CANNOT be made to work on all displays — keyboard events reach only the key window and key is exclusive (one per app), and a global key monitor is refuted on macOS 26 (Input Monitoring, finding #3 above). So the mouse paths are THE cancel, the "click empty space to cancel" text stays visible on every overlay, and Esc is best-effort (fires only where a window is key).

**SHIPPED (all four slices, owner-verified on 3-display + single-display).** Poller in `macos.rs` (`picker_cursor_start`/`stop`, CoreGraphics FFI, on-change throttle, stops on every picker exit — select/cancel/Esc — via the `finish()` choke point). Overlays self-map global CG points to local and drive hover (Slice 2) and select/cancel (Slice 3); `setFocus` is gone from the picker path. The cancel-instruction hint is vertically centered (not bottom-anchored) so the Dock can't occlude it on any edge — important because Esc no longer cancels. Diagnostic probes (`focus_probe`/`try_activate_probe`) and all `/tmp` logging removed in Slice 4. Plan: `docs/WINDOW-PICKER-POLLER-PLAN.md`.
