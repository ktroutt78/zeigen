...
**(2) Motion blur does NOT deliver at 0.6s/2x (blind test).** The "V3 zoom looks better" case had narrowed to motion blur. A blinded ladder below "subtle" (half/quarter/eighth + two no-blur, shuffled, key withheld) was owner-judged: **favorite was a no-blur clip; owner imagined a difference between two byte-identical no-blur clips; owner rejected every strength they could perceive.** At a 0.6s ramp to 2x there is no strobe worth fixing. Motion blur is OUT of the value case. Code stays (`main.swift`, `BLUR=on`, radial CIZoomBlur, `radius = floor + k*|v|`) as **off-by-default insurance** for faster/bigger zooms only.

**Corrected thesis (inherit this).** V3 is a **performance/thermal rewrite** (encoder-bound, hardware-uniform, no fanless-Air throttle) that also gives cleaner hard edges and untouched non-zoomed frames (V2's whole-timeline oversample softens non-zoomed frames to ~43 dB; V3 leaves them identity at ~53 dB). It is **NOT** the "buttery/expensive zoom" upgrade it was framed as. Also settled this session: Phase 1 color round-trip needs CI color management OFF (`workingColorSpace = NSNull`) — CI's managed 709 round-trip lifts luma ~2.75 levels. **OPEN:** owner deciding whether perf + cleaner-edges + non-zoomed justifies the Phase 3-4 overlay-porting rounds; V3 not wired into the app (V2 remains default).
