...
- **Waveform alignment drift (Phase 10 regression)** — Surfaced during Phase 12 c2 verification (2026-05-20). Review-window waveform peaks don't line up with the playhead position where the audio actually plays. For S > 0 (audio track starts after video) the peak draws earlier (LEFT) of the playhead for audio-times below a per-recording crossover and flips direction after; for S = 0 the peak draws later (RIGHT) of the playhead. Root cause: `Waveform.tsx` linearly maps `audioBuffer.duration` across canvas width that actually represents `videoElement.duration` (via the parent Timeline's `playPct`/trim percentages). The audio track is consistently shorter than the video track because the last mic CMSampleBuffer reaches the writer before the last video frame (mic startup lag + end-of-recording asymmetry); ffprobe across pre-c2 and post-c2 recordings showed V−A spanning ~30–650ms (worst case observed: 644ms on `recording-2026-05-19-225852` scratch). WebKit's `decodeAudioData` honors the audio track edit list, so `audioBuffer`-time 0 corresponds to video-time `S` (audio track `start_time`) — but the AudioBuffer doesn't expose `S`, so the component can't correct from the buffer alone. Diagnostic snapshot from `recording-2026-05-20-160744.mp4`: `audioBuffer.duration` = 8.768s, `videoElement.duration` = 8.870s, ffprobe audio `start_time` = 0.068s, ffprobe audio track duration = 8.832s. Pre-existing bug, not a c2 regression — same drift behavior observed on saved pre-c2 recordings like `recording-2026-05-20-074640.mp4` (V−A = 254ms). **Proposed fix needs design discussion before implementation**, two directions on the table: (a) pad the audio track with silence on save so it matches the video track exactly — Waveform then needs no V/S logic, but this changes save-time file size and pipeline behavior across every MP4 export (LinkedIn, Copy, plain Save); (b) thread `videoElement.duration` plus an inferred `S` into the Waveform component and draw buckets across the `[S, S+A] / V × W` canvas range — surgical, but requires getting `S` into the frontend somehow (ffprobe via a Rust command, an MP4 atom parse in JS, or an assumption derived from V−A and the engine's known leading-gap behavior). Not in Phase 12 scope.
- **Timeline zoom on trim** — when trim handles are set, the timeline rescales to show only the trimmed region across full width (waveform, playhead, scrubbing all operate on the zoomed view). Revert control restores full timeline. Trim handles become dual-purpose: define export region AND define focus region. Surfaced during Phase 12 c2 verification, 2026-05-20.
- **Trim handle hit target too small** — the 10px-wide `TrimHandle` is hard to grab consistently; clicks even slightly off-target fall through to the track's `onTrackPointerDown` and seek instead of starting a drag. Surfaced during Phase 13 c3 verification (2026-05-20). Likely fix: transparent padding wrapper at ~24px wide around the visible 10px bar so the cursor doesn't have to be pixel-perfect. Pre-existing — not a c3 regression.
- **Scrub preview thumbnail clips playhead grab dot** — the `ScrubPreview` thumbnail sits high enough that its top edge overlaps the white playhead circle at the top of the timeline. Cosmetic. Bump thumbnail down a few pixels or shift the playhead circle. Surfaced during Phase 13 c3 verification (2026-05-20).
- **Aggressive arnndn NR over-suppresses speech when typing** — surfaced during Phase 14 c2 UAT (2026-05-21). The bundled RNNoise model treats keyboard transients as broadband signal; when typing overlaps speech, the model's voice-activity detection misclassifies and suppresses speech instead of just the keyboard noise. Pre-existing behavior — c2 didn't change the NR model, only made the over-suppression audible before save (which is exactly the c2 win). Possible directions: (1) swap the arnndn model for one trained on keyboard-noise + speech overlap, (2) apply NR as a dry/wet blend instead of full replacement, (3) pair with the still-queued capture-side limiter so peak transients don't dominate the model's VAD input, (4) make NR toggleable per-recording in the Review window. Real design discussion needed before implementation.
- **Bubble lags cursor in saved video (clock-reference mismatch)** — surfaced during Phase 15 c3 hands-on verification (2026-06-03). When the bubble is dragged during recording, playback shows the bubble following the cursor on a small fixed-time delay (~30-80ms typical). Pre-existing — c2 byte-stability test confirms saved files are identical to Phase 14 outputs, so this lag has been there since the bubble-drag feature shipped (Phase 3.5). Made more visible by Phase 15 c3 because long recordings are practical now, so more bubble-drag content reaches review. **Direction:** bubble lags cursor (the saved video at any screen PTS shows the bubble position from ~SCK_LAG ms earlier). **Magnitude:** constant in TIME (≈ SCK init lag, recording-/machine-dependent), linear-in-pixels with drag speed. **Root cause:** `bubble_position_event` (`lib.rs:332`) timestamps each entry as `now - rec.started_at`, where `rec.started_at = Instant::now()` is captured at the `engine_start` IPC handler invocation (`lib.rs:186`) — BEFORE the engine command roundtrip and SCK stream setup. `screen.mp4` PTS starts at 0 at the first SCK frame, which arrives ~30-80ms later. Composite (`composite.rs:507-530`) builds overlay expressions assuming bubble `t` == screen PTS; no offset compensation exists for the SCK init lag (cf. `WEBCAM_LEAD_MS` which exists for the webcam stream but not the bubble log). Possible fix options (decision open — both have tradeoffs):
  - **(A) Calibrated SCK_LEAD_MS constant** — subtract a fixed offset from bubble entry `t` when building the composite overlay expression. Same shape as the existing `WEBCAM_LEAD_MS` pattern. ~10 lines in `composite.rs`. **Risk:** SCK init lag is NOT constant across recordings/machines (varies with display config, system load, AVCaptureSession state). A fixed constant will over- or under-correct run-to-run; could trade a consistent ~50ms lag for a swinging ±30ms error. Calibration done on one machine won't generalize.
  - **(B) Per-recording first-frame anchor** — engine emits a `first_frame_time` event when SCK delivers frame 0; the Rust handler stores that wall-clock instant on `ActiveRecording`. `bubble_position_event` then computes `t = now - first_frame_time` (instead of `now - started_at`). Composite reads accurate per-recording bubble timestamps without any constant. Requires Swift engine change (emit the event) + Rust handler wiring + a brief "pending first frame" window where bubble logs queue or get discarded. **Trade-off:** more code, more accurate, future-proof against SCK behavior changes.
  - Other directions worth considering when this gets picked up: engine could write bubble entries directly to its own internal clock and emit them on the same channel as frames (changes the API contract); composite could probe the screen.mp4's first-frame PTS at export time and subtract that (the container `start_time=0.043` we observed during c3 verification is suggestive but not definitive of SCK lag). Both are heavier refactors than (B).
  - Verification approach when this gets picked up: clap test (calibration recording with a visible hand strike + audible clap) as the ground-truth reference, plus instrumentation of `first_frame_time - started_at` across a sweep of recordings to characterize the SCK lag distribution. Don't commit to (A) without that distribution measured.
- **Preview-only ~20ms mouth lag from webcam play() startup latency** — surfaced during Phase 15 c3 hands-on verification (2026-06-03), measured via instrumentation 2026-06-03 / 2026-06-07. After the `onTimeUpdate -> w.play()` fix lands the webcam continuously, the c3 sync layer measures a stable `drift_ms ≈ -22` (first session) / `-18` (subsequent sessions) — webcam.currentTime sits ~20ms behind `target = screen.currentTime - LEAD` for the entire playback. Not growing, not oscillating. **Source:** the first `onTimeUpdate -> w.play()` call has a short startup gap before the webcam decoder actually begins advancing; in that ~500ms gap the webcam advances ~20ms less than the screen, then both lock at perfect 1× rate afterward. The 20ms delta becomes the persistent offset for the rest of the session. **Saved file unaffected** — composite at export uses ffmpeg `tpad`, not a live `<video>.play()`. **Backlog only — user accepted as sub-perceptible** (within ±40ms lip-sync tolerance) on 2026-06-03. Any naive fix (e.g., tightening the 50ms align threshold to 25ms) would reintroduce a visible stutter every few seconds to chase a sub-perceptible delta. If this gets prioritized: pre-seek the webcam slightly AHEAD of target by an estimated play-startup-latency before calling play(), or schedule a one-shot align ~500ms after first play() to absorb the gap without ongoing thrashing. Don't tighten the steady-state threshold.
- **Webcam-vs-mic-audio sync regression — intermittent, observed across capture-engine-v2 history** — surfaced 2026-06-07. User reports saved-file webcam mouth vs mic audio was **solidly in sync for an extended period on capture-engine-v2** (good enough that v2 became the daily-driver build), then later **regressed to visibly off**, then most-recent clap + speaking tests **look in sync again**. The was-good → broke → good pattern is incompatible with the obvious "WEBCAM_LEAD_MS=280 is miscalibrated for this hardware" theory — a wrong fixed constant would be consistently off across all recordings on the same machine, not intermittently. So the timing change is most likely a **specific commit on capture-engine-v2 that altered webcam stream alignment** (composite tpad value, audio_shift logic, engine first-frame handling, or webcam ffmpeg start sequencing), reverted or masked by a later commit, and that's why the symptom comes and goes. **No fix now** — user can live with current state. If the desync recurs and becomes reproducible: **git-bisect across the capture-engine-v2 history against a clap-test recording**, since we now have a reliable clap-based measurement method (visible hand strike + audio peak, count frame offset). Do the bisect FIRST, before assuming this is calibration territory or jumping to option (B) of the bubble-lag entry above. Once a suspect commit is identified, the fix scope is whatever that commit did — could be a one-line revert, could need (B)-style first-frame anchoring on the webcam stream too. Don't pre-design the fix from the wrong premise.

### V2.3 (inherited from v1.0): process-lifecycle cleanup

Two v1.0 main-branch defects discovered 2026-05-25 during a disk-usage investigation. Both affect v1.0 daily-driver use; V2.3 inherits and fixes. Not in V2.2 scope (capture-engine refactor) — do not fix now.

- **BACKLOG-V2.3-1: Webcam ffmpeg orphan protection** — A webcam-capture ffmpeg subprocess from a 2026-05-20 recording kept writing to `~/Movies/Zeigen/.scratch/recording-2026-05-20-213553/sources/webcam-00.mp4` for 5 days after the parent engine exited; final size **187GB**. Likely root cause: the engine process exited (crash, force-quit, or normal stop with broken cleanup) before signaling the webcam ffmpeg child to terminate; the child reparented to launchd and kept writing. Fix scope (V2.3): register the webcam ffmpeg as a death-signal child of the engine. macOS options — process group + signal-on-shutdown; kqueue `EVFILT_PROC` parent-watch in a tiny wrapper; periodic parent-PID poll with self-terminate. No V2.2 fix — the webcam compositor is outside the capture-engine refactor scope.
- **BACKLOG-V2.3-2: Scratch sweeper on engine startup** — same incident. The v1.0 lifecycle assumes every recording exits via the Review window (discard or save), but engine-binary invocations from the harness, UATs, or direct calls bypass that path, so scratch dirs accumulate without bound. Fix scope (V2.3): on engine startup (`main.swift`, before `.ready`), scan `~/Movies/Zeigen/.scratch/` for `recording-*` dirs older than 24h, log a warning per stale dir, and delete it. Belt-and-suspenders for BACKLOG-V2.3-1 plus accumulated cruft from non-Tauri invocations. Threshold is conservative: anything older than a day is a forgotten test artifact or a zombie — real in-progress recordings finish in minutes.

### V2.3: harness run-provenance (process risk)

Surfaced 2026-05-27 during V2.2 c3 report composition. A measurement-provenance risk in the drift harness, not a v1.0 defect — but V2.3 owns the harness next, so it lands here.

- **BACKLOG-V2.3-3: Acceptance-run numbers don't survive in the repo** — The c2 acceptance drift numbers survive only in commit body `ac5e115`: `harness/v2-drift/output/` is gitignored, so the endSession-removal control experiment (2026-05-25) overwrote the raw acceptance run. Process risk, not a nit — authoritative measurement data that a later phase depends on (V2.4's R2 gate-close decision reads the c2 residual) shouldn't live solely in a commit message, where it can't be re-`ffprobe`d or re-aggregated. Fix scope (V2.3): either un-gitignore an `output/acceptance/` archive dir, or have the harness write acceptance runs to a committed path, so authoritative numbers are reproducible from the repo rather than reconstructed from git log.
...
- A/V sync drift (surfaced in Phase 5 UAT) — separate follow-up, own diagnosis pass needed. Sharpened during Phase 13 c3 verification (2026-05-20): after c3 collapsed the variable V−A drift, a residual **fixed** ~43ms offset remains regardless of recording length (peak renders LEFT of playhead by ~3px at typical canvas widths, ~43ms in time). Constant across recordings — no longer a function of position-in-recording. ffprobe shows AAC `skip_samples=2092` side-data on every recording — at 48kHz that's exactly 43.6ms of leading priming the AAC decoder is supposed to discard. Working hypothesis: WKWebView's `decodeAudioData` strips priming (so `audioBuffer.duration` matches ffprobe-reported A), but the `<video>` element plays back the priming as audible silence, delaying real audio by ~43ms relative to the container time index that the waveform renders against. Likely the same constant offset users felt as "voice doesn't quite line up with video" in Phase 5 UAT. Possible fixes (need design pass): (a) subtract priming-offset from `audioStart` in Waveform — requires exposing `skip_samples` via the ffprobe command alongside `start_time`, (b) re-encode audio at save time with `-af apad` or equivalent to absorb the offset, (c) engine-side change to encode without priming (probably impossible — AAC frame-aligned encoders always emit priming). (a) is the surgical option; the others reopen pipeline shape decisions. **Re-confirmed during Phase 15 c3 verification (2026-06-02):** flagged by user as a suspected c3 regression on a short save. Diagnosis showed saved-file drift unchanged from Phase 14 — c2 byte-stability test passes bit-exact on both fixtures under HEAD `8812ffa`, and ffprobe on the new saved file (V=0.034, A=0.000, `skip_samples=1024`) matches the Phase 14 baseline fixtures exactly. The save pipeline actually halves the raw screen's 44ms priming to 21ms via the h264_videotoolbox re-encode — both behaviors identical pre- and post-Phase-15. The user perception of "newly broken" comes from Phase 15 c3 making long recordings practical (Stop→preview is now instant on 12+ min clips), so users review and save more recordings and the always-present ~21ms drift surfaces more often. Not a c3 regression. Surgical fix (a) still the right direction when this gets prioritized.
