...
- **Review preview audio parity with saved files** — c3 applies arnndn at save, but in-review playback uses raw scratch. Users can't audibly verify NR didn't over-suppress speech (the 18:04 anomaly path) before sharing. Possible approaches: (1) WASM-based RNNoise applied client-side during playback, (2) background-generated preview file with NR applied, served to the review window, (3) hybrid with on-demand toggle. Real design discussion needed before implementation. Surfaced 2026-05-20 after Phase 12 ship.
